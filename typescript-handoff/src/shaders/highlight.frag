...
uniform float uBrightness; uniform float uContrast;
varying vec2 vUv;
...
void main() {
  ...
  vec3 graded = ((c.rgb - 0.5) * uContrast + 0.5) * uBrightness;
  float lum = dot(graded, vec3(0.299, 0.587, 0.114));
  float word = inBox(vUv, uBox, 0.004) * uStrength;
  vec3 desat = mix(graded, vec3(lum), uDesat * (1.0 - word)) * (1.0 - 0.12 * uDesat * (1.0 - word));
